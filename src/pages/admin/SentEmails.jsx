...
import { Eye, MoreVertical, RotateCcw, Send, Trash2, X, XCircle, CheckCircle2 } from "lucide-react";
...
export function SentEmailsPage() {
  ...
  function toggleMenu(invite, e) {
    ...
    const MENU_HEIGHT = 4 * 38 + 12; // approximate menu height
  ...
  return (
    <div className="p-6 md:p-10 max-w-7xl mx-auto min-h-screen bg-slate-50/50 font-sans">
      ...
      {detailsInvite && (
        ...
        >
          <div
            className="bg-white rounded-3xl shadow-[0_20px_60px_rgba(0,0,0,0.15)] max-w-lg w-full max-h-[90vh] overflow-y-auto"
          ...
          >
            <div className="sticky top-0 bg-white/80 backdrop-blur-md px-8 py-5 border-b border-gray-100 flex justify-between items-center z-10">
             ...
             <dl className="px-8 py-6 grid gap-y-5">
               ...
               <DetailRow label="Name they entered">
                 {detailsInvite.filled_name || <span className="text-gray-400 italic">not opened yet</span>}
               </DetailRow>
               ...
               <DetailRow label="Admin Notified (40% Progress)">
                 {detailsInvite.admin_notified ? (
                   <span className="text-emerald-600 font-medium flex items-center gap-1.5"><CheckCircle2 size={14} /> Yes (Email sent)</span>
                 ) : "No"}
               </DetailRow>
               <DetailRow label="Product tour progress">
                ...
                {detailsInvite.progress_steps?.length > 0 && (
                  <p className="mt-1 text-xs text-gray-500">
                    <span className="font-semibold text-gray-700">Completed:</span> {detailsInvite.progress_steps.map((s) => STEP_LABELS[s] || s).join(", ")}
                  </p>
                )}
              ...
              <DetailRow label="Times opened">
                <span className="font-semibold">{detailsInvite.use_count}</span>
                <span className="text-gray-500 ml-1">{detailsInvite.single_use ? "(single-use link)" : "(multi-use link)"}</span>
              </DetailRow>
              <DetailRow label="Device lock">
                {detailsInvite.device_lock
                  ? <span className="text-gray-700 font-medium">Restricted to the first device that opened it</span>
            ...
            <div className="px-8 py-5 border-t border-gray-100 bg-gray-50/50 flex justify-end">
              <button className="px-6 py-2.5 rounded-xl bg-white border border-gray-200 text-gray-700 font-semibold hover:bg-gray-50 transition-colors shadow-sm" onClick={() => setDetailsInvite(null)}>
                Close
              </button>
            </div>
          </div>
