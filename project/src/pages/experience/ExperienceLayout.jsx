...
export function ExperienceLayout() {
  ...
  // Lets an admin revoking the invite kick out an already-open tab — otherwise
  // login is only checked once at magic-link consume and a revoked session would
  // keep working indefinitely (see /api/invites/:id/status).
  ...
  useEffect(() => {
    ...
    const check = () => {
      apiGet(`/api/invites/${inviteId}/status`)
        .then((data) => {
          if (data?.status === "revoked") setRevoked(true);
        })
        .catch(() => {
          // Best-effort — a failed check shouldn't interrupt the experience.
        });
    };
  ...
  if (revoked) {
    resetExperience();
    return (
      <div className="flex min-h-dvh flex-col items-center justify-center gap-3 text-center px-4">
        <p className="text-lg font-semibold text-foreground">Access revoked</p>
        ...
        <div className="flex items-center justify-between border-b border-border px-5 py-4">
          <div className="flex items-center gap-2">
            <div className="relative inline-flex size-7 items-center justify-center shrink-0">
              ...
              <Sparkle className="relative size-3.5 text-white" weight="fill" />
            </div>
