...
function StoryModal({ story, onClose }) {
  ...
  // when an admin hasn't filled in the dedicated testimonial fields yet.
  const personName = story.person_name || story.company;
      ...
      >
          ...
          <div className="grid gap-10 md:grid-cols-[minmax(0,1fr)_280px] items-center">
            <div className="flex gap-5 items-start">
              ...
              <div>
                ...
                <p className="mt-4 text-sm font-semibold text-foreground">{personName}</p>
                {personTitle && <p className="text-xs text-primary font-medium">{personTitle}</p>}
              </div>
            </div>
        ...
        <div className="flex items-center justify-between border-t border-border px-6 py-4 shrink-0">
          {story.download_url ? (
            ...
            >
              <Download className="size-4" /> {story.download_label || "Download PDF"}
            </a>
          ) : (
            <span />
          )}
