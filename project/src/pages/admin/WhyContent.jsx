...
export function WhyTab({ productSlug }) {
  ...
  const [gumletId, setGumletId] = useState("");
  ...
  useEffect(() => {
    if (pageData) {
      ...
      setGumletId(pageData.extra?.gumletId || "");
    }
  }, [pageData]);
  ...
  async function handleSave(e) {
    ...
    try {
      await apiSend(
        ...
        "PUT",
        { headline, description, extra: { videoTitle, videoDuration, videoUrl, gumletId } },
        token,
      );
      setSaveMsg("Saved.");
      refetchPage();
        ...
        <form onSubmit={handleSave}>
          {pageLoading ? (
            ...
            <>
              ...
              <div className="form-group">
                <label htmlFor="why-gumlet-id">Gumlet Video ID</label>
                <input
                  id="why-gumlet-id"
                  className="form-control"
                  value={gumletId}
                  onChange={(e) => setGumletId(e.target.value)}
                  placeholder="e.g. 67c11c363e62e53909989066"
                />
                <p style={{ fontSize: "0.75rem", color: "var(--slate-500)", marginTop: 4 }}>
                  Paste just the ID or the full play.gumlet.io/embed/… link — either works. Takes
                  priority over Video URL if both are given.
                </p>
              </div>
