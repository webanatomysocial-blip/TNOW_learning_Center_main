...
router.post("/api/magic/:code/consume", async (req, res) => {
  try {
    ...
    const exhausted = invite.single_use || (invite.usage_limit != null && newUseCount >= invite.usage_limit);
    await db("email_invites")
      .where({ id: invite.id })
      .update({
...
router.post("/api/magic/:code/request-access", async (req, res) => {
  try {
    ...
    const reason = ["expired", "revoked"].includes(requestedReason) ? requestedReason : "device";
